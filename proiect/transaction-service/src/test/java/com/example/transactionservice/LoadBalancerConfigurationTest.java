...
class LoadBalancerConfigurationTest {
    ...
    @Test
    @DisplayName("AccountFeignClient uses service name 'account-service' and no hardcoded URL")
    void testFeignClientUsesServiceName() {
        FeignClient feignClient = AccountFeignClient.class.getAnnotation(FeignClient.class);
        assertThat(feignClient).isNotNull();
        assertThat(feignClient.name()).isEqualTo("account-service");
        assertThat(feignClient.url()).isBlank();
    }

    @Test
    @DisplayName("Spring Cloud RoundRobinLoadBalancer distributes requests across two account-service instances")
    void testRoundRobinLoadBalancerDistributesBetweenTwoReplicas() {
        ServiceInstance inst1 = new DefaultServiceInstance(
                "account-service:8082", "account-service", "localhost", 8082, false);
        ServiceInstance inst2 = new DefaultServiceInstance(
                "account-service:8182", "account-service", "localhost", 8182, false);
        ...
        var loadBalancer = new RoundRobinLoadBalancer(new SimpleObjectProvider<>(supplier), "account-service");
